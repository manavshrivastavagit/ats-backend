""" Defines the candidate repository """
...
class candidateRepository:
    """ The repository for the candidate model """

    @staticmethod
    def get(last_name, first_name):
        """ Query a candidate by last and first name """
        try:
            return candidate.query.filter_by(last_name=last_name, first_name=first_name).one()
    ...
    def getByEmail(email):
        """ Query a candidate by email """
        try:
            return candidate.query.filter_by(email=email).one()
        except NoResultFound:
            return None
            return {
                "status": "error",
    ...
    def getById(id):
        """ Query a candidate by last and first name """
        try:
            return candidate.query.filter_by(id=id).one()
        except:
            # logger.error("{} has multiple records in Marcotti database for: {}".format(model.__name__, conditions))
            return {
                "status": "error",
    ...
    def getAllIds(page_number=1, per_page_data=10):
        ...
        try:
            ...
            record_query = candidate.query.paginate(int(page_number), int(per_page_data), error_out=False)
            record_query_dicts = []
    ...
    def update(self, id, last_name, first_name,
               ...
               last_updated_date):
        """ Update a candidate """
        candidate = self.getById(id=id)
        # candidate.city = city if city else "bangalore"
        ...
        if alt_email is not None:
            candidate.alt_email = alt_email
        if alt_phone_no is not None:
            candidate.alt_phone_no = alt_phone_no
        ...
        if profile_picture_url is not None:
            candidate.profile_picture_url = profile_picture_url
    ...
    def delete(id):
        """ delete a candidate """
        try:
            del_id = candidate.query.filter_by(id=id).one()
    ...
    def create(last_name, first_name,
        ...
        newcandidate = candidate(last_name=last_name, first_name=first_name,
                   email=email,
                   password=password,
                   alt_email=alt_email,
